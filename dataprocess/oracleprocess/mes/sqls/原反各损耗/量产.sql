...
ON a.MATERIALNO = b.ITEM_NUMBER
	...
	GROUP BY DEVICE ) aa
	INNER JOIN
	(SELECT 料號,SUM(數量) AS touru FROM offline.loss_yuanfan_input WHERE 業務日期 BETWEEN 'starttime' AND 'endtime' AND (工單 LIKE '___M%' OR 工單 LIKE '___P%' OR 工單 LIKE '___T%') GROUP BY 料號
	) bb
