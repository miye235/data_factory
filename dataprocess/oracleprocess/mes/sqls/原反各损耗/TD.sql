...
ON a.MATERIALNO = b.ITEM_NUMBER
	...
	GROUP BY DEVICE ) aa	
	INNER JOIN 
	(SELECT 料號,SUM(數量) AS touru FROM offline.loss_yuanfan_input WHERE 業務日期 BETWEEN 'starttime' AND 'endtime' AND 工單 like '___D%' GROUP BY 料號
	) bb
	ON aa.device = bb.料號 ) aaa
	INNER JOIN
	(SELECT 料號,SUM(數量) AS chanchu,業務日期  FROM offline.loss_yuanfan_output WHERE 業務日期 BETWEEN 'starttime' AND 'endtime' AND 工單 like '___D%' 
	GROUP BY 料號,業務日期
	) bbb
	 ON aaa.料號 = bbb.料號) s