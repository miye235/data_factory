select cbi.LOT,vlm.WO,dd.REASON,dd.DESCR,dd.QUANTITY AS dianque,tt.bin,tt.QUANTITY as agui,oo.TRANSACTIONTIME as checkouttime,oo.TRANSACTION,
...
decode(substr(vlm.DEVICE,2,2), 'BO', '京东方', 'CE', '熊猫(CEC)','HK', '惠科', 'CS', '华星', 'SP', '盛波光电', 'CH', '彩虹光电', 'SS', '三星', 'CP', '成都熊猫') as kehu,
...
decode(substr(vlm.WO,1,1), 'Q', '重庆', 'K', '昆山') as changqu,

--case when substr(vlm.WO,1,1) = 'Q' then '重庆'
--     when substr(vlm.WO,1,1) = 'K' then '昆山'
--     else ''
--     end as changqu,
...
group by cbi.LOT ,vlm.DEVICE,AA.WO, vlm.MATERIAL, vlm.LOTNO,vl1.MATERIAL,vmm.MATERIALNO,vmm.CUSTLOT,vl1.DEVICE,vlm.WO,vlm.LOTTYPE,AA.FABCODE,vl1.FABCODE,dd.REASON,dd.DESCR,dd.QUANTITY,tt.bin,tt.QUANTITY,oo.TRANSACTIONTIME,oo.TRANSACTION
order by tt.bin